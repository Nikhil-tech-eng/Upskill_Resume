...
const Summary = () => text(d.summary) ? <Section title="Profile" className="rt-summary"><p>{d.summary}</p></Section> : null;
  ...
  return <article className="rt-23 resume-template"><header className="rt-header"><div><div className="rt-eyebrow">CURRICULUM VITAE</div><h1>{p.name || 'Your Name'}</h1><h2>{p.title}</h2></div><div className="rt-contact">{[p.email,p.phone,p.location,p.linkedin,p.github].filter(Boolean).map((x,i)=><span key={i}>{x}</span>)}</div></header><div className="rt-body"><div className="rt-layout"><main><Summary/><Experience/><Projects/></main><aside><Skills/><Education/><Certifications/></aside></div></div></article>;
}
