...
export default function ClassicProfessionalTemplate({ template, personal = {}, summary = "", skills = [], education = [], experience = [], projects = [], certifications = [] }) {
  ...
  return (
    <div className="resume-preview-page classic-preview" style={{ "--template-accent": template.accent, "--template-secondary": template.secondary }}>
      <div className="classic-header">
        <h1>{getSafeText(personal.fullName) || "Your Name"}</h1>
        {personal.professionalTitle && <p>{personal.professionalTitle}</p>}
      ...
      {summary && (
        <section className="resume-preview-section classic-section">
          <h3>Summary</h3>
          <p>{summary}</p>
        </section>
      )}
      ...
      {visibleSkills.length > 0 && (
        <section className="resume-preview-section classic-section">
          <h3>Skills</h3>
          <div className="resume-skill-list classic-skills">
            {visibleSkills.map((skill, index) => <span key={`${skill}-${index}`}>{skill}</span>)}
          </div>
        </section>
      )}
      ...
      {safeExperience.length > 0 && (
        <section className="resume-preview-section classic-section">
          ...
          {safeExperience.map((item, index) => (
            <div key={`exp-${index}`} className="resume-entry classic-entry">
      ...
      {safeEducation.length > 0 && (
        <section className="resume-preview-section classic-section">
          ...
          {safeEducation.map((item, index) => (
            <div key={`edu-${index}`} className="resume-entry classic-entry">
              <div className="resume-entry-header">
      ...
      {safeProjects.length > 0 && (
        <section className="resume-preview-section classic-section">
          ...
          {safeProjects.map((item, index) => (
            <div key={`project-${index}`} className="resume-entry classic-entry">
              <div className="resume-entry-header">
