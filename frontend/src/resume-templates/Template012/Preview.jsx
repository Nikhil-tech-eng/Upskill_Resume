...
const Certifications=({d})=>A(d.certifications).length?<Section t="Certifications">{A(d.certifications).map((x,i)=><article className="entry" key={i}><div><h4>{S(x,['name','title','certificate'])}</h4><div className="org">{S(x,['issuer','organization','provider'])}</div></div><time>{S(x,['date','issueDate','year'])}</time></article>)}</Section>:null;
 ...
 return <article className="resume-template rt100-12">
 <header className="identity"><div className="index">12</div><div className="name"><h1>{p.name||'Your Name'}</h1><h2>{p.title}</h2></div><Contact p={p}/></header>
 <div className="body"><div className="single"><Experience d={d}/><Summary d={d}/><Projects d={d}/><Skills d={d}/><Education d={d}/><Certifications d={d}/></div></div>
 </article>;
}
